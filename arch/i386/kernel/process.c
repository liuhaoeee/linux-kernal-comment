...
/* rebooting needs to touch the page at absolute addr 0 */
	...
	//这个涉及到系统的冷启动和热启动。BIOS在执行过程中要测试内存单元0x472
	//的内容，如果这个值是0就要执行对内存等的自检，称为“冷启动”（linux内核源
	//代码情景分析中说是热启动，我觉得应该是冷启动）。而若为 0x1234则跳过
	//自检，称为热启动
	*((unsigned short *)0x472) = 0x1234;
	for (;;) {
		for (i=0; i<100; i++) {
			...
			for(j = 0; j < 100000 ; j++)
				/* nothing */;
			//0x64端口是i8042键盘控制器的控制端口，0xfe命令字的意思是将
			//P32-P21三个针脚拉为低电平，持续6usec。这段代码的实际效果就
			//相当于你按下机箱上的 RESET键。
...
//copy_thread用来拷贝父进程的系统堆栈并做相应的调整
	...
	struct task_struct * p, struct pt_regs * regs)
{
	...
	p->tss.fs = USER_DS;
	p->tss.gs = KERNEL_DS;
	//正是下面两行代码，设置了内核态堆栈，使系统每次进入内核态都自动切换到此内核堆栈
	p->tss.ss0 = KERNEL_DS;
