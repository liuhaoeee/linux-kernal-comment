...
/*
		如果当前（前台）虚拟控制台终端运行于UTF-8模式，则要把UTF-8代码还原
		成16位的Unicode，最后存放在tc变量中
*/
		if (utf) {
...
/*
			对于虚拟控制台，系统的图形卡工作于字符模式，最终写入图形卡的是一个
			16位短字，其低字节是字符本身的代码，高字节则为字符的属性，字符的属性
			决定着显示该字符时的亮度、颜色等要素。所以，最后还得把字符转换成用于
			图形卡的8位代码。这种转换是由conv_uni_to_pc()完成的
			对于每个可显示的字符，图形卡在显示器上显示的是这个字符的特殊图形，称为
			“字模”--glyph，工作于字符模式而不是图形模式的VGA图形卡可以显示256个字模
			所以只能用于拼音文字。根据正在使用的语言，可以将不同的字模装入图形卡，然后
			将这种语言中可以显示的字符的Unicode变换成这些字符的编号。这样就可以实现
			“本地化”的目的。
			conv_uni_to_pc()就是实现从Unicode从到字模编号的变换或者说映射。
*/
		    tc = conv_uni_to_pc(c);
		    if (tc == -1 || tc == -2)
		      continue;
			...
			ok = 0;
		        ...
		        ok = 1;
/*一般的8位字符要通过一个数组变换成Unicode*/
		} else {	/* no utf */
/*
			#define translate	(vc_cons[currcons].d->vc_translate)
			每个虚拟控制台的vc_data数据结构中有个指针vc_translata，指向
			一个数组为256的数组，8位的ASCII或其它的代码可以通过这个数组转换
			成16位的Unicode。
			内核中有个二维数组translations[][]，定义于consolemap.c,这个
			二维数组中包含着4个字符转换数组，每个虚拟控制台的指针vc_translate
			指向其中的一个。可以通过系统调用ioctl()来改变。不仅如此，还可以通过
			ioctl()从用户空间下载或上载用户定义的字符转换数组来覆盖原有的数组，
			也可以下载或上载用户定义的字库。虚拟控制台的ioctl()函数vt_ioctl()
			是个堪称巨型的函数，代码量达700行之多，提供了各种各样的功能，/usr/bin
			目录下的setfont、setkeycodes、loadkeys等工具就是建立在这个系统调用
			之上的。
*/
		    tc = translate[toggle_meta ? (c|0x80) : c];
		    ok = 0;
		}
/*tc中是待显示的16位Unicode，而c中则是该字符原来的8位代码*/
		 ...
		 */
/*
		并不是所有字符都是可以显示的。如果转换前的字符的数值小于32，即“空格”
		那就是控制字符，一般情况下是不能显示的。如果字符可以显示（ok==1）
		虚拟控制台又处于正常工作状态，就可以进一步处理字符的显示了。
*/
		...
		if (vc_state == ESnormal && ok) {
			...
			if (decim)
				insert_char(currcons);
			//通过scr_writew()写入图形卡的是一个16位短字，其低字节是字符本身的代码tc
			//高字节则是字符的属性attr。
/*
			#define scr_writew(val, addr) (*(addr) = (val))
			PC机图形卡如VGA上的RAM区间在0xa0000以上，BIOS以下的位置上，其相对
			于起点的地址与显示屏上的位置有着一一对应的关系，CPU可以通过访问内存指
			令直接访问。由于图形卡工作于字符方式，所以只要把字符的代码，实际上是
			字模的编号，连同其属性写入相应的内存存储单元即可，而不必关心具体的字模
			和像素。
			显然，linux内核在本地化方面的努力只限于拼音文字，而不适用于CJKV，以汉字
			为例，如果要在内核中支持汉字，图形卡就必须工作于图像方式，这样才能显示数千
			个常用汉字的字模。此外，即使是拼音文字，现代的图形用户界面也要求采用图像方式
*/
 ...
 */
//这个函数的作用是为当前的虚拟终端分配缓冲区和有关的数据结构，用来记录该
//虚拟终端的上下文。
int con_open(struct tty_struct *tty, struct file * filp)
{
	unsigned int	idx;
