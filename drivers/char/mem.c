...
//memory_open实际上实现了一个分配器（根据次设备号区分各个设备，
//并且选择适当的file_operations）
static int memory_open(struct inode * inode, struct file * filp)
{
	switch (MINOR(inode->i_rdev)) {
		case 0:
			...
			break;
		case 1:
			filp->f_op = &mem_fops; // /dev/mem,物理内存
			break;
		case 2:
			filp->f_op = &kmem_fops; // /dev/kmem,内核虚拟地址空间 
			break;
		case 3:
			filp->f_op = &null_fops; // /dev/null,比特位桶 
			break;
		case 4:
			filp->f_op = &port_fops; // /dev/port,访问I/O端口
			break;
		case 5:
			filp->f_op = &zero_fops; // /dev/zero,WULL字符源 
			break;
		case 7:
			filp->f_op = &full_fops;
			break;
...
/*
	字符设备的初始化由chr_dev_init()完成，包括对内存、终端、打印机、
	鼠标等字符设备的初始化。块设备的初始化由blk_dev_init()完成，这
	包括对IDE硬盘、软盘、光驱等块设备的初始化
*/
...
#endif
/*
 *      Rude way to allocate kernel memory buffer for tape device(磁带设备)
 */
#ifdef CONFIG_FTAPE
