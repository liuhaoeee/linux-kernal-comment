...
/*
	CD_ROM现在是PC机最常用的设备之一，在Linux中，也提供了多种支持CD_ROM的设备驱动。
	CD_ROM主要有两种类型IDE接口、SCSI接口，这里主要讨论IDE接口。市场上有多种品牌的
	CD_ROM，各个开发商对各自的CD_ROM实现的功能有不同，同时对同一功能，不同的CD_ROM
	实现的方法也有不同。Linux提供了标准的IDE接口的CD_ROM驱动（driver\block\ide-cd.c），
	只要是与ATAPI 1.2标准兼容的CD_ROM，该驱动都能用。有些CD_ROM生产商即提供了与ATAPI
	兼容的，也提供了自己特有的驱动。如果设备使用自己特有的设备驱动，则IDE-CD驱动就不工
	作。生产商提供的自己特有的驱动在drivers\cdrom\目录下。
*/
