...
/*
	...
	linux_binfmt构成的一个链表。而对不同格式二进制文件的处理程序则
	通过注册在相应的linux_binfmt中的函数来实行，实际这里已经使用了
	面向对象的思想，而这种情况在linux的内核源码中经常可以看到
*/
...
//“共享库”、即动态连接库映像的装入则由load_elf_library()完成
/*
	实际上共享库的映像也是二进制的，但是一般说“二进制”映像是指带有main()函数的、
	可以独立运行并构成一个进程主体的可执行程序的二进制映像。另一方面，尽管装入/
	启动二进制映像的过程中蕴含了共享库的装入(否则无法运行)，但是在此过程中却并
	没有调用load_elf_library()，而是通过别的函数进行，这个函数只是在sys_uselib()、
	即系统调用uselib()中通过函数指针load_shlib受到调用。所以，load_elf_library()
	所处理的是应用软件在运行时对于共享库的动态装入，而不是启动进程时的静态装入
*/
static int load_elf_library(int fd){
    struct file * file;
	struct elfhdr elf_ex;
	...
	/* First of all, some simple consistency checks */
	//映像类型必须是ET_EXEC或ET_DYN 机器(CPU)类型必须相符
