...
/*
	每个进程除了有一个进程ID之外，还有一个进程组。进程组是一个或多个进程的集合。
	它们与同一作业相关联，可以接受来自同一终端的各种信号。每个进程组都有唯一的
	进程组ID
	每个进程组都可以有一个组长进程。组长进程的标识是，其进程组ID等于进程ID。组
	长进程可以创建一个进程组，创建该组中的进程，然后终止。只要在某个进程组中有一
	个进程存在，则该进程组就存在，与其组成进程是否终止无关。进程组的最后一个进程
	可以终止，或者转移到另一个进程组
	进程可以调用setpgid来加入一个现有的组或者一个新进程组
	一个进程只能为它自己或它的子进程设置进程组ID，在它的子进程调用了exec函数后，
	它就不再能改变该子进程的进程组ID
	setpgid函数将pid进程的进程组ID设置为pgid。如果这两个参数相等，则由pid指定
	的进程变成进程组组长；如果pid是0，则使用调用者的进展ID；如果pgid是0，则由pid
	指定的进程ID将用于进程组ID
	在大多数作业控制的shell中，在fork之后调用setpgid函数，使父进程设置其子进程的
	进程组ID，并且使子进程设置其自己的进程组ID
*/
//设置指定进程pid的进程组号为pgid
...
found_task:
	//先要检查做这个操作的权限: 当前进程只能将自身和其子进程设置为进程组id,并且 
	//当前进程和其子进程必须属于同一次会话 (同组的进程一定属于同一次会话) 
	if (p->p_pptr == current || p->p_opptr == current) {
		...
		return -ESRCH;
	//如果是一次会话的leader,也不可以，注意进程组的首领进程也是可以改变组id的
	if (p->leader)
		return -EPERM;
	
	//要设进程号不是要设定的组号,如果是,直接设,因为这意味着是增加了以自己的pid作为新的组号的进程组,这个 
	//进程也将成为新进程组的首领进程,所以在此根本不用比较会话号,自己对自己肯定是同一次会话.如果条件不满足
	//则要做这些判断
	if (pgid != pid) {
		struct task_struct * tmp;
		//能不能找到一个进程,组号正好是要设定的组号,并且和要设定的进程属于同一个控制台(同一个会话) 
		//找到才可以设定,其实这里就是要判定组号的合法性, 即必须是一个已经存在的组,而且和当前同一次
		//会话才可以操作,这个也不能忘记,其实就是说:同组的进程一定属于同一次会话
		for_each_task (tmp) {
			if (tmp->pgrp == pgid &&
...
/*
	...
	如果该调用进程已经是一个进程组的组长，则此函数返回出错。为了保证不会发生这种情况，
	通常先调用fork，然后使其父进程终止，而子进程则继续。

	如果调用进程不是进程组的领头进程, 该函数才能建立新的会话. 调用 setsid 之后, 进程成为新会话的领头进程. 
	进程成为新进程组的领头进程.进程失去控制终端

	想使进程丢弃控制终端，可以使用setsid调用，使得当前进程创建一个新的session和一个新的进程组，当前进
	程为新的session的会话头进程，新的进程组的进程组头进程。。。不过如果当前进程已经是一个进程组的头进
	程的话，setsid会失败
*/
/*
		控制终端 
...
{
	//如果当前进程是已经是一个会话首领进程, 则不能建立一个新的会话
	if (current->leader)
		return -EPERM;
	//设置当前进程为新会话首领
	current->leader = 1;
	current->session = current->pgrp = current->pid;	//创建该会话的进程号赋值成为会话号
	current->tty = NULL;	//表示该领头进程没有控制终端，使当前进程失去控制终端
