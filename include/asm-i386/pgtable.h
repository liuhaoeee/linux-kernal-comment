...
/*
	Linux的三级分页结构
	l         页表是从线性地址向物理地址转换中不可缺少的数据结构，而且它使用的频率较高。页表必须存放在物理存储器中。
	l         虚存空间有4GB，按4KB页面划分页表可以有1M页。
	若采用一级页表机制，页表有1M个表项，每个表项4字节，
	这个页面就要占用4MB的内存空间。
	l         由于系统中每个进程都有自己的页表，如果每个页表占用4MB，
	   对于多个进程而言就要占去大量的物理内存，这是不现实的。
	l         在目前用户的进程不可能需要使用4GB这么庞大的虚存空间，
	   若使用1M个表项的一级页表，势必造成物理内存极大的浪费。
	l         为此，Linux采用了三级页表结构，以利于节省物理内存。
	l         三级分页管理把虚拟地址分成四个位段：
	   页目录、页中间目录、页表、页内偏址。
	l         系统设置三级页表系列：
	...
	l         三级分页结构是Linux提供的与硬件无关的分页管理方式。
	l         当Linux运行在某种机器上时，需要利用该种机器硬件的存储管理机制来实现分页存储。
	l         Linux内核中对不同的机器配备了不同的分页结构的转换方法。
	l         对x86，提供了把三级分页管理转换成两级分页机制的方法。
	l         其中一个重要的方面就是把PGD与MGD合二为一，使所有关于PMD的操作变为对PGD的操作
*/
/*
...
//定义了一个物理页面可改变的属性屏蔽码 PAGE_MASK低12位全为0 再与上页的访问位标志和脏标志 就可以得到一个新的pte项，见pte_modify函数
...
//修改页表项的保护属性位 主要是accessed位和dirty位
	...
	if (pmd_none(*pmd)) {
		...
		if (pmd_none(*pmd)) {
			if (page) {
				...
				mem_map[MAP_NR(page)] = MAP_PAGE_RESERVED;	//和pte_alloc()函数的唯一区别
				return page + address;
			}
...
{
	//将给的线性地址转换成其所属页表项页面表的下标---即高位的第二个10位，PGD_SHIFT为12.PTRS_PER_PTE为1024
	address = (address >> PAGE_SHIFT) & (PTRS_PER_PTE - 1);
	if (pmd_none(*pmd)) {
		pte_t * page = (pte_t *) get_free_page(GFP_KERNEL);
		//在get_free_page函数期间，可能导致睡眠...
		if (pmd_none(*pmd)) {
			if (page) {
				pmd_val(*pmd) = _PAGE_TABLE | (unsigned long) page;
				return page + address;
