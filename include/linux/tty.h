...
/*
	参见tty_io.c中flush_to_ldisc()中对flip的使用
	
	filp：空翻
	缓冲区char_buf[]的大小是TTY_FLIPBUF_SIZE的两倍，就好像
	一张纸的两面。与char_buf[]相平行的还有一个数组flag_buf[]
	一个字符都是与一个flag字节相匹配，不过从put_queue()调用
	tty_insert_flip_char()时总是把flag字节设成0。指针char_buf_ptr
	和flag_buf_ptr都是给tty_insert_flip_char()使用的，而flush_to_ldisc()
	中对这些指针的设置就起着把纸翻一个面的作用。但实际上也将缓冲区的容量减少了
	一半，而tty_insert_flip_char()在发现缓冲区已满的情况下就会把接受到的字符
	丢弃
*/
...
/*
	在不同的系统中或者不同的键盘上使用的控制字符有可能不同，例如，在屏幕上显示大量的
	信息时，一般可以按Ctrl_S暂停屏幕上的信息向上滚动，然后可以按Ctrl_Q继续输出。
	但是，在有些键盘上也许不是Ctrl-S和Ctrl-Q，而是别的什么，所以，在termios中
	还有个数组c_cc[]，用来定义各种控制字符的代码
*/
...
//表示如果接受到CTRL_S字符就暂停输出，知道接受到CTRL_Q再恢复输出
...
//表示如果接受到CTRL+C等控制字符就向在该终端上运行的进程发送信号
#define L_ISIG(tty)	_L_FLAG((tty),ISIG)
//规范模式，即加工模式
#define L_ICANON(tty)	_L_FLAG((tty),ICANON)
#define L_XCASE(tty)	_L_FLAG((tty),XCASE)
//表示“本地回送”，就是在从终端设备接受到一个字符时就立即将该字符同送到该终端设备上
...
struct tty_struct {
	int	magic;
	//每种终端设备都有自己的tty_driver结构，结构中主要是一些函数指针，确定了
	//对具体终端类型的一整套操作。
	struct tty_driver driver;
	struct tty_ldisc ldisc;
/*
	每个逻辑意义上的终端设备接口，如串行口、图形卡和键盘的组合乃至终端设备的
	两端，都有一个termios数据结构。这个数据结构在某种意义上可以看作是对
	tty_ldisc结构的补充，它规定了对接口上输入和输出的每个字符所做的处理
	以及传输的速度即“波特率”。
*/
	struct termios *termios, *termios_locked;
	int pgrp;	//终端所关联的进程组号，在终端接受到如Ctrl+C等信号时会根据此关联组号将信号发送给这个组
	int session;
	dev_t	device;
	unsigned long flags;
	int count;
...
/*
	tty_flip_buffer数据结构是终端设备的输入缓冲区，底层的中断服务
	程序将接受到的字符存储于这个缓冲区中，供上层读取。
*/
...
#define N_TTY_BUF_SIZE 4096
	...
	int num_overrun;
/*
	与flip相应的另一个成分是字符位图process_char_map，这个字符位图中的
	每一位都代表着一个8位字符，所以位图的大小为32字节。如果某个字符在这个
	位图中对应位为1,就表示可能要对这个字符作出一些特殊的处理或反应（往往
	只是在“加工模式”下起作用）
*/
