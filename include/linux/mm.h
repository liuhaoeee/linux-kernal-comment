...
struct vm_area_struct {
	...
	unsigned long vm_end;	//vm_start和vm_end 虚拟区域的开始和终止地址
	pgprot_t vm_page_prot;	//虚存区域的页面的保护特性
	unsigned short vm_flags;	//vm_flags指出了虚存区域的操作特性：
								//	VM_READ              虚存区域允许读取
								//	VM_WRITE             虚存区域允许写入
								//	VM_EXEC             虚存区域允许执行
...
/* more */	
	struct vm_operations_struct * vm_ops;	//vm_ops是指向vm_operations_struct结构体的指针。该结构体中包含着指向各种操作的函数的指针
	unsigned long vm_offset;	//vm_offset是该区域的内容相对于文件起始位置的偏移量，或相对于共享内存首址的偏移量
	struct inode * vm_inode;	//若虚存区域映射的是磁盘文件或设备文件的的内容，则vm_inode指向这个文件的inode结构体，否则vm_inode为NULL
 ...
 */
 //Linux虚拟存储管理对于虚存区的操作定义在vm_operation_struct数据结构中，
 //通过在vm_area_struct结构中使用指针vm_ops来确定该虚存区可以进行的一系列操作
struct vm_operations_struct {
	void (*open)(struct vm_area_struct * area);	//打开操作，当内核生成一个虚存区后或者当虚存区被复制后，就用该命令打开
	void (*close)(struct vm_area_struct * area);	//关闭操作，当内核销毁一个虚存区时，就调用该命令
	void (*unmap)(struct vm_area_struct *area, unsigned long, size_t);	//取消映射操作，当内核取消虚存区的部分或者全部映射时，调用该命令；当取消全部映射后，内核就会自动调用close()进行关闭操作。
	...
	unsigned long (*nopage)(struct vm_area_struct * area, unsigned long address,
		unsigned long page, int write_access);	//处理缺页异常，当进程访问一个不属于内存的有效页面时，就会调用该命令，返回该页的物理地址
	unsigned long (*wppage)(struct vm_area_struct * area, unsigned long address,
		unsigned long page);	//处理写保护异常，当往一个被保护的页面上写入数据时，就会调用该命令
...
#define GFP_ATOMIC	0x01	//分配内存优先级高，不会睡眠
#define GFP_USER	0x02
#define GFP_KERNEL	0x03	//常用的方式，可能会阻塞
