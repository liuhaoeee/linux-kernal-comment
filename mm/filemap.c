	...
	unsigned long page, int no_share)
{
	...
	address &= PAGE_MASK;
	block = address - area->vm_start + area->vm_offset;	//获取address地址处对应的文件中的块地址
	block >>= inode->i_sb->s_blocksize_bits;	//将block转化为块数
	i = PAGE_SIZE >> inode->i_sb->s_blocksize_bits;
	p = nr;
	do {
		//具体的文件系统只有支持此函数才能支持mmap
		*p = bmap(inode,block);	//应该是通过逻辑块号得到实际块号 bmap->block map 将inode对应的文件中的逻辑块号1 2 3 转化为磁盘上对应的实际块号
		i--;
		block++;
		p++;
	} while (i > 0);
...
/*
 * NOTE! mmap sync doesn't really work yet. This is mainly a stub（存根） for it,
	...
	unsigned long page)
{
	...
	bh = buffer_pages[MAP_NR(page)];	//swap_cashe数组
	if (bh) {
		/* whee.. just mark the buffer heads dirty */
		struct buffer_head * tmp = bh;
		do {
			mark_buffer_dirty(tmp, 0);
			tmp = tmp->b_this_page;		/* circular list of buffers in one page */
		} while (tmp != bh);
		return;
	}
...
int generic_mmap(struct inode * inode, struct file * file, struct vm_area_struct * vma)
{
	...
	//如果虚拟地址空间映射的文件开始部分没有对块对齐 返回EINVAL
	if (vma->vm_offset & (inode->i_sb->s_blocksize - 1))
		return -EINVAL;
	//如果i节点对应的超级块不存在或者文件不是普通文件 则返回EACCES
